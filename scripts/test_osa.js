function run() {
  ...
  var functionsCode = `
    ...
    function computeSearchMatchScore(queryWord, targetText) {
      ...
      const allQueryWords = Object.assign({}, window.FOOD_VECTORS || {}, window.QUERY_VECTORS || {});
      ...
      if (window.FOOD_VECTORS) {
        let maxSemanticSim = 0;
          ...
          for (let i = 0; i < matchedTargetWords.length; i++) {
            ...
            if (!isSubsumed) {
              if (!validTargetWords.includes(matchedTargetWords[i].word)) {
                validTargetWords.push(matchedTargetWords[i].word);
              }
            }
          }
  ...
  var score2 = computeRecipeSemanticScore(recipe2, "ウニ");
  
  return "ウニ score1: " + score1 + "\\nウニ score2: " + score2;
}
